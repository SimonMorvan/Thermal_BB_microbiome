...
#### ßdiv - Pseudorep  ####
load(file ="ps.bacunmerged.tree.RData")

#### I Hellinger distance ####
ps.fun.hell <- transform_sample_counts(ps.bacunmerged.tree, function(x) sqrt(x/sum(x)))
...
PCA_Hell <- plot_ordination(ps.fun.hell, ord.PCA.hellinger, type="samples",color="Plot",shape="Burning_intensity")+ 
  theme_bw()+
  ggtitle(label = "PCA - Hellinger distance - 16S ")+
...
ps_clr <- microbiome::transform(ps.bacunmerged.tree, "clr")        
...
PCA_Ait <- plot_ordination(ps_clr, ord_clr, type="samples",color="Plot",shape="Burning_intensity") + 
  theme_bw()+
  ggtitle(label = "PCA - Aitchison distance - 16S")+
  geom_point(colour = "black", size = 4.5) +
  geom_point(size = 4)+
  geom_text_repel(aes(label=Sample_ID), color="black")+
  theme(axis.text=element_text(size=12))+ # size of lables, axis titles
...
ord.PCoA.Unifrac <- ordinate(ps.bacunmerged.tree, method="PCoA", distance = "uunifrac")

PCoA_Unifrac <- plot_ordination(ps.bacunmerged.tree, ord.PCoA.Unifrac, type="samples", color = "Plot",shape="Burning_intensity")+ 
  theme_bw()+
  ggtitle(label = "PCoA - Unweigthed Unifrac distance - 16S ")+
  geom_point(colour = "black", size = 4.5) +
  geom_point(size = 4)+
  geom_text_repel(aes(label=Sample_ID), color="black")+
  theme(axis.text=element_text(size=12))+ # size of lables, axis titles
...
ord.PCoA.wUnifrac <- ordinate(ps.bacunmerged.tree, method="PCoA", distance = "wunifrac")

PCoA_wUnifrac <- plot_ordination(ps.bacunmerged.tree, ord.PCoA.wUnifrac, type="samples", color = "Plot",shape="Burning_intensity")+ 
  theme_bw()+
