...
setwd("/storage/Simon_morvan/Reads/ch2/ITS/02_DADA2_obj")
path <- ("/storage/Simon_morvan/Reads/ch2/ITS/")  # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
...
fnFs <- sort(list.files(path, pattern="_R1.fastq", full.names = TRUE)) # Forwards reads
...
sample.names <- sapply(strsplit(basename(fnFs), "_R"), `[`, 1) # Same sample name for reverse and forward reads
...
track <- cbind(out, sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab.nochim),rowSums(seqtab.nochim.bin))
...
setwd("/storage/Simon_morvan/Reads/ch2/ITS/02_DADA2_obj")
save(taxa_unite_fun,file="taxa_unite_fun.RData")
...
setwd("/storage/Simon_morvan/Reads/ch2/ITS/02_DADA2_obj")
save(taxa_unite_euk,file="taxa_unite_euk.RData")
